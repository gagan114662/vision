...
class TestTermNetAgent:
    ...
    def test_is_gpt_oss_model(self, agent):
        with patch("termnet.agent.CONFIG", {"MODEL_NAME": "gpt-oss-mini"}):
            assert agent._is_gpt_oss_model() is True

        with patch("termnet.agent.CONFIG", {"MODEL_NAME": "llama-3"}):
            assert agent._is_gpt_oss_model() is False

    def test_has_gpt_oss_tool_call(self, agent):
        text_with_call = "<|start|>assistant<|channel|>commentary to=terminal_execute<|message|>{}<|call|>"
        assert agent._has_gpt_oss_tool_call(text_with_call) is True

        text_without_call = "Regular text without tool call"
        assert agent._has_gpt_oss_tool_call(text_without_call) is False

    def test_parse_gpt_oss_tool_calls(self, agent):
        text = '<|start|>assistant<|channel|>commentary to=terminal_execute<|constrain|>json<|message|>{"cmd":["ls"]}<|call|>'
        calls = agent._parse_gpt_oss_tool_calls(text)

        assert len(calls) == 1
        assert calls[0]["name"] == "terminal_execute"
        assert calls[0]["args"]["command"] == "ls"

    def test_parse_gpt_oss_tool_calls_multiple(self, agent):
        text = """
        <|start|>assistant<|channel|>commentary to=terminal_execute<|message|>{"cmd":["pwd"]}<|call|>
        <|channel|>commentary to=terminal_execute<|message|>{"cmd":["ls"]}<|call|>
        """
        calls = agent._parse_gpt_oss_tool_calls(text)

        assert len(calls) == 2
        assert calls[0]["args"]["command"] == "pwd"
        assert calls[1]["args"]["command"] == "ls"
