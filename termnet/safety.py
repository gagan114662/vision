...
# Reasonable defaults to satisfy tests (mirrors your JSON config)
SECRET_PATTERNS = [
    r"(?i)(api[_-]?key|apikey)\s*[:=]\s*['\"]?([\w\-]+)",
    r"(?i)(secret|token|password|passwd|pwd)\s*[:=]\s*['\"]?([\w\-]+)",
    r"(?i)bearer\s+([\w\.\-]+)",
    r"(?i)(aws[_-]?access[_-]?key[_-]?id|aws[_-]?secret[_-]?access[_-]?key)\s*[:=]\s*['\"]?([\w\-]+)",
    r"(?i)(github[_-]?token|gh[_-]?token)\s*[:=]\s*['\"]?([\w\-]+)",
    r"sk-[A-Za-z0-9]{24,}",
    r"ghp_[A-Za-z0-9]{20,}",
    r"ghs_[A-Za-z0-9]{20,}",
]
...
def redact_secrets(text: str) -> str:
    ...
    for pat in SECRET_PATTERNS:
        try:
            rx = re.compile(pat)
            redacted = rx.sub(
                lambda m: m.group(0).split(m.groups()[-1])[0] + REDACTION, redacted
            )
        except re.error:
            # best-effort: skip bad regex
            continue
    return redacted
...
class SafetyChecker:
    ...
    def sanitize_output(self, text: str) -> str:
        # Use the stronger redaction function
